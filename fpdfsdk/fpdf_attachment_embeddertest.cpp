...
TEST_F(FPDFAttachmentEmbedderTest, WriteAttachmentDescription) {
  ...
  EXPECT_EQ(2f * sizeof(kTestValue), FPDFAttachment_GetAttachmentDesc(valid_attachment, buf.data(),
                                                                 buf.size()));
  EXPECT_EQ(kTestValue, GetPlatformWString(buf.data()));

  EXPECT_EQ(2f * sizeof(kTestValue), FPDFAttachment_GetAttachmentDesc(desc_absent, buf.data(),
                                                                 buf.size()));
  EXPECT_EQ(kTestValue, GetPlatformWString(buf.data()));

  EXPECT_EQ(2f * sizeof(kTestValue), FPDFAttachment_GetAttachmentDesc(numeric_desc, buf.data(),
                                                                 buf.size()));
  EXPECT_EQ(kTestValue, GetPlatformWString(buf.data()));
}
