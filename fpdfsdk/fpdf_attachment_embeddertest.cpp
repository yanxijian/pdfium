...
TEST_F(FPDFAttachmentEmbedderTest, WriteAttachmentDescription) {
  ...
  // If /Desc is not present or a different type, the API still updates it and
  // writes it to the description.
  EXPECT_TRUE(
  ...
  EXPECT_EQ(14u, FPDFAttachment_GetAttachmentDesc(valid_attachment, buf.data(),
                                                  buf.size()));
  EXPECT_EQ(kTestValue, GetPlatformWString(buf.data()));

  EXPECT_EQ(14u, FPDFAttachment_GetAttachmentDesc(desc_absent, buf.data(),
                                                  buf.size()));
  EXPECT_EQ(kTestValue, GetPlatformWString(buf.data()));

  EXPECT_EQ(14u, FPDFAttachment_GetAttachmentDesc(numeric_desc, buf.data(),
                                                  buf.size()));
  EXPECT_EQ(kTestValue, GetPlatformWString(buf.data()));
}
