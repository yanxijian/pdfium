...
FPDFAttachment_SetAttachmentDesc(FPDF_ATTACHMENT attachment,
  ...
  ByteString byte_string_value =
      UNSAFE_BUFFERS(ByteStringFromFPDFWideString(value));
  file_spec_dict->SetNewFor<CPDF_String>(ByteString("Desc"), byte_string_value);
  return true;
}
...
FPDFAttachment_GetAttachmentDesc(FPDF_ATTACHMENT attachment,
  ...
  RetainPtr<const CPDF_Object> object =
      file_spec_dict->GetObjectFor(ByteStringView("Desc"));
  if (!object || (!object->IsString() && !object->IsName())) {
    // Per API description, return an empty string in these cases.
    return Utf16EncodeMaybeCopyAndReturnLength(WideString(), buffer_span);
  }
  RetainPtr<const CPDF_String> string_object = ToString(object);
  return Utf16EncodeMaybeCopyAndReturnLength(string_object->GetUnicodeText(),
                                             buffer_span);
}
