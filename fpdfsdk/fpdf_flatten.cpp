...
void PruneFieldArray(CPDF_Array* fields,
  ...
  for (size_t i = fields->size(); i > 0; --i) {
    ...
    if (!prune_field) {
      ...
      if (kids && visited_fields.insert(field.Get()).second) {
        PruneFieldArray(kids.Get(), widget_annots, visited_fields, level + 1);
        prune_field = kids->IsEmpty();
      }
    }
    if (prune_field) {
...
void RemoveFlattenedFields(CPDF_Document* document,
  ...
  std::set<const CPDF_Dictionary*> visited_fields;
  PruneFieldArray(fields.Get(), widget_annots, visited_fields, /*level=*/0);
