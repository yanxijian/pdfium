...
void RemoveSharedWidgetsFromSet(
  ...
  for (int i = 0, page_count = document->GetPageCount();
    ...
    std::set<RetainPtr<const CPDF_Dictionary>> other_page_widget_annots =
        CollectPageWidgetAnnots(page_dict.Get());
    if (other_page_widget_annots.empty()) {
      continue;
    }

    std::set<RetainPtr<const CPDF_Dictionary>> page_only_widget_annots;
...
bool PruneFieldArray(
  ...
  for (size_t i = fields->size(); i > 0; --i) {
    ...
    bool prune_field = pdfium::Contains(
        widget_annots, RetainPtr<const CPDF_Dictionary>(field.Get()));
    if (!prune_field) {
