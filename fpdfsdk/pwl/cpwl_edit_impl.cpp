...
bool CPWL_EditImpl::Iterator::GetWord(CPVT_Word& word) const {
  ...
  if (vt_iterator_->GetWord(word)) {
    word.set_start_location(edit_->VTToEdit(word.start_location()));
    return true;
  }
  return false;
}
  ...
  while (pIterator->NextWord()) {
    ...
    if (pIterator->GetWord(word)) {
      if (bSelect) {
        ...
        if (pFillerNotify->IsSelectionImplemented()) {
          CFX_FloatRect rc(word.start_location().x,
                           line.ptLine.y + line.fLineDescent,
                           word.start_location().x + word.width(),
                           line.ptLine.y + line.fLineAscent);
          rc.Intersect(rcClip);
          pFillerNotify->OutputSelectedRect(pSystemData, rc);
        } else {
          CFX_Path pathSelBK;
          pathSelBK.AppendRect(word.start_location().x,
                               line.ptLine.y + line.fLineDescent,
                               word.start_location().x + word.width(),
                               line.ptLine.y + line.fLineAscent);
      ...
      if (bContinuous) {
        ...
        DrawTextString(
            pDevice,
            CFX_PointF(word.start_location().x + ptOffset.x,
                       word.start_location().y + ptOffset.y),
...
void CPWL_EditImpl::ScrollToCaret() {
  ...
  CPVT_Line line;
  if (pIterator->GetWord(word)) {
    ptHead.x = word.start_location().x + word.width();
    ptHead.y = word.AscentY();
    ptFoot.x = word.start_location().x + word.width();
...
void CPWL_EditImpl::RefreshWordRange(const CPVT_WordRange& wr) {
  ...
  while (pIterator->NextWord()) {
    ...
    if (place.LineCmp(wrTemp.BeginPos) == 0 ||
      ...
      CFX_FloatRect rcWord(wordinfo.start_location().x,
                           lineinfo.ptLine.y + lineinfo.fLineDescent,
                           wordinfo.start_location().x + wordinfo.width(),
                           lineinfo.ptLine.y + lineinfo.fLineAscent);
...
void CPWL_EditImpl::SetCaretOrigin() {
  ...
  CPVT_Line line;
  if (pIterator->GetWord(word)) {
    caret_point_.x = word.start_location().x + word.width();
    caret_point_.y = word.start_location().y;
  } else if (pIterator->GetLine(line)) {
