    ...
    if (pIterator->GetWord(word)) {
      if (bSelect) {
        ...
        if (pFillerNotify->IsSelectionImplemented()) {
          CFX_FloatRect rc(word.location().x, line.ptLine.y + line.fLineDescent,
                           word.GetCaretX(), line.ptLine.y + line.fLineAscent);
          rc.Intersect(rcClip);
          pFillerNotify->OutputSelectedRect(pSystemData, rc);
        } else {
          CFX_Path pathSelBK;
          pathSelBK.AppendRect(
              word.location().x, line.ptLine.y + line.fLineDescent,
              word.GetCaretX(), line.ptLine.y + line.fLineAscent);
      ...
      if (bContinuous && !word.is_rtl()) {
        if (sTextBuf.IsEmpty() || place.LineCmp(oldplace) != 0 ||
            font_index != nFontIndex || crOldFill != crCurFill) {
          if (!sTextBuf.IsEmpty()) {
  ...
  while (pIterator->NextWord()) {
    ...
    if (place.LineCmp(wrTemp.BeginPos) == 0 ||
      ...
      CFX_FloatRect rcWord(
          ...
          wordinfo.GetCaretX(), lineinfo.ptLine.y + lineinfo.fLineAscent);
