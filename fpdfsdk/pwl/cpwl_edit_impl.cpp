...
bool CPWL_EditImpl::Iterator::GetWord(CPVT_Word& word) const {
  ...
  if (vt_iterator_->GetWord(word)) {
    word = CPVT_Word(word.word(), word.charset(), word.word_place(),
                     edit_->VTToEdit(word.pt()), word.ascent(), word.descent(),
                     word.width(), word.font_index(), word.font_size());
    return true;
  }
  return false;
}
  ...
  while (pIterator->NextWord()) {
    ...
    if (pIterator->GetWord(word)) {
      if (bSelect) {
        ...
        if (pFillerNotify->IsSelectionImplemented()) {
          CFX_FloatRect rc(word.pt().x, line.ptLine.y + line.fLineDescent,
                           word.pt().x + word.width(),
                           line.ptLine.y + line.fLineAscent);
          rc.Intersect(rcClip);
          pFillerNotify->OutputSelectedRect(pSystemData, rc);
        } else {
          CFX_Path pathSelBK;
          pathSelBK.AppendRect(word.pt().x, line.ptLine.y + line.fLineDescent,
                               word.pt().x + word.width(),
                               line.ptLine.y + line.fLineAscent);
      ...
      if (bContinuous) {
        ...
        DrawTextString(
            pDevice,
            CFX_PointF(word.pt().x + ptOffset.x, word.pt().y + ptOffset.y),
...
void CPWL_EditImpl::ScrollToCaret() {
  ...
  CPVT_Line line;
  if (pIterator->GetWord(word)) {
    ptHead.x = word.pt().x + word.width();
    ptHead.y = word.AscentY();
    ptFoot.x = word.pt().x + word.width();
...
void CPWL_EditImpl::RefreshWordRange(const CPVT_WordRange& wr) {
  ...
  while (pIterator->NextWord()) {
    ...
    if (place.LineCmp(wrTemp.BeginPos) == 0 ||
      ...
      CFX_FloatRect rcWord(wordinfo.pt().x,
                           lineinfo.ptLine.y + lineinfo.fLineDescent,
                           wordinfo.pt().x + wordinfo.width(),
                           lineinfo.ptLine.y + lineinfo.fLineAscent);
...
void CPWL_EditImpl::SetCaretOrigin() {
  ...
  CPVT_Line line;
  if (pIterator->GetWord(word)) {
    caret_point_.x = word.pt().x + word.width();
    caret_point_.y = word.pt().y;
  } else if (pIterator->GetLine(line)) {
