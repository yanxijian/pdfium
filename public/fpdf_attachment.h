...
// dictionary of the embedded file |attachment| to the string value.
//
//   attachment - handle to an attachment.
...
// Returns true if successful.
FPDF_EXPORT FPDF_BOOL FPDF_CALLCONV
FPDFAttachment_SetAttachmentDesc(FPDF_ATTACHMENT attachment,
                                 FPDF_WIDESTRING value);

// Experimental API.
// Gets the string value corresponding to "/Desc" in the file specification
// dictionary of the embedded file |attachment|. Similarly to the above APIs,
// |buffer| is only modified if |buflen| is longer than the length of the
// string's value. If the key doesn't exist, or is not a string then 2 is
// returned by the API, alongside an empty string. Other errors return 0
// instead.
//
//   attachment - handle to an attachment.
//   buffer     - buffer for holding the string value encoded in UTF-16LE.
...
FPDF_EXPORT unsigned long FPDF_CALLCONV
FPDFAttachment_GetAttachmentDesc(FPDF_ATTACHMENT attachment,
                                 FPDF_WCHAR* buffer,
                                 unsigned long buflen);
