...
constexpr size_t kMaxDecodeSize = 64 * 1024 * 1024;

}  // namespace

DataAndBytesConsumed BrotliDecoder::Decode(
  ...
  if (estimated_decode_size > kMaxDecodeSize) {
    return {DataVector<uint8_t>(), 0u};
  }
  ...
  switch (result) {
    ...
    case BROTLI_DECODER_RESULT_NEEDS_MORE_OUTPUT:
      if (estimated_decode_size != kMaxDecodeSize) {
        uint32_t new_decode_size = std::min(estimated_decode_size * 2, static_cast<uint32_t>(kMaxDecodeSize));
        return Decode(src_span, params, new_decode_size);
      }
      return {DataVector<uint8_t>(), 0u};
  }
}
