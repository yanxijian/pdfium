...
bool JpegDecoder::Rewind() {
  ...
  // scale_denom_ was validated in Create() and, for non-MCU-aligned images,
  // reset to 1 in InitDecode() (called from Create(), and again just above when
  // re-decoding), so it is safe to use as-is here.
  ...
  // The scanline buffer was sized (via CalcPitch()) for output_width_ columns,
