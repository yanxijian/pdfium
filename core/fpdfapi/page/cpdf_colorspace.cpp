...
void CPDF_ICCBasedCS::TranslateImageLine(pdfium::span<uint8_t> dest_span,
  ...
  if (!bTranslate) {
    ...
    if (nPixelCount.IsValid()) {
      bTranslate =
        nPixelCount.ValueOrDie() < static_cast<int>(nMaxColors.ValueOrDie()) * 3 / 2;
    }
  }
  if (bTranslate) {
  ...
  if (cache_.empty()) {
    ...
    size_t src_index = 0;
    for (int i = 0; i < nMaxColors.ValueOrDie(); i++) {
      uint32_t color = i;
      uint32_t order = static_cast<int>(nMaxColors.ValueOrDie()) / kNumQuantizedLevels;
    ...
    if (profile_->IsSupported()) {
      profile_->TranslateScanline(cache_, temp_src, nMaxColors.ValueOrDie());
    }
  }
