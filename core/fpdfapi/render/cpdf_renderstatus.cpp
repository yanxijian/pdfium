...
bool CPDF_RenderStatus::ProcessTransparency(CPDF_PageObject* pPageObj,
  ...
  auto bitmap_device = CFX_RenderDevice::CreateForNewBitmapWithBackdrop(
  ...
  if (bTextClip) {
    ...
    auto text_device = CFX_RenderDevice::CreateForBitmap(text_mask_bitmap);
    CHECK(text_device);
...
RetainPtr<CFX_DIBitmap> CPDF_RenderStatus::GetBackdrop(
  ...
  auto device = CFX_RenderDevice::CreateForBitmap(backdrop);
  CHECK(device);
  ...
  for (size_t iChar = 0; iChar < textobj->GetCharCodes().size(); ++iChar) {
    ...
    if (!pType3Char->LoadBitmapFromSoleImageOfForm()) {
      ...
      if (fill_alpha == 255) {
        ...
        auto bitmap_device = CFX_RenderDevice::CreateForNewBitmap(
            rect.Width(), rect.Height(), FXDIB_Format::kBgra);
        if (!bitmap_device) {
          return true;
        }
