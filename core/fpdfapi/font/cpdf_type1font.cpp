...
bool CPDF_Type1Font::Load() {
  base14_font_ = fxge::GetStandardFontName(&base_font_name_);
  if (!IsBase14Font()) {
    return LoadCommon();
  }
  ...
  if (base14_font_ == fxge::StandardFont::kSymbol) {
    base_encoding_ = FontEncoding::kAdobeSymbol;
  } else if (base14_font_ == fxge::StandardFont::kDingbats) {
...
bool CPDF_Type1Font::IsSymbolicFont() const {
  return base14_font_.has_value() && fxge::IsSymbolicFont(base14_font_.value());
}

bool CPDF_Type1Font::IsFixedFont() const {
  return base14_font_.has_value() && fxge::IsFixedFont(base14_font_.value());
}
