...
void CPDF_TextPage::ProcessFormObject(CPDF_FormObject* form_obj,
  ...
  const CPDF_PageObjectHolder* pHolder = form_obj->form();
  for (auto it = pHolder->begin(); it != pHolder->end(); ++it) {
    ...
    if (page_obj->IsText()) {
      ProcessTextObject(page_obj->AsText(), actual_form_matrix, pHolder, it);
...
void CPDF_TextPage::ProcessTextObject(
  ...
  if (IsSameAsPreTextObject(text_obj, obj_list, ObjPos)) {
    return;
  }
...
CPDF_TextPage::GenerateCharacter CPDF_TextPage::ProcessInsertObject(
  ...
  if (obj->CharCount() == 1 && IsHyphenCode(current_char) &&
