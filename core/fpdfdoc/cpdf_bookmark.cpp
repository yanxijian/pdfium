...
ColorReturn CPDF_Bookmark::GetColor() const {
  ...
  RetainPtr<CPDF_Array> RGB_array = dict_->GetArrayFor("C");
  if (!RGB_array || RGB_array.size() != 3) {
    return {false, {0.0f, 0.0f, 0.0f}};
  }
  float red = RGB_array.GetNumberAt(0).Get().GetFloat();
  float green = RGB_array.GetNumberAt(1).Get().GetFloat();
  float blue = RGB_array.GetNumberAt(2).Get().GetFloat();
