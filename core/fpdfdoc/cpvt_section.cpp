  ...
  if (nTotalLines > 0) {
    ...
    for (int32_t l = 0; l < nTotalLines; l++) {
      ...
      for (const auto& segment : bidi) {
        bool bRTL = (segment.direction == CFX_BidiChar::Direction::kRight ||
                     (segment.direction == CFX_BidiChar::Direction::kNeutral &&
                      eCurrentDirection == CFX_BidiChar::Direction::kRight));

        int32_t start =
            bRTL ? segment.start + segment.count - 1 : segment.start;
        int32_t end = bRTL ? segment.start - 1 : segment.start + segment.count;
        int32_t step = bRTL ? -1 : 1;
        ...
        for (int32_t i = start; i != end; i += step) {
          ...
          if (fxcrt::IndexInBounds(word_array_, w)) {
            ...
            pWord->nDirection = bRTL ? CPVT_WordDirection::kRightToLeft
                                     : CPVT_WordDirection::kLeftToRight;
