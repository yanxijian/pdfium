...
ByteString GenerateBorderAP(const CFX_FloatRect& rect,
  ...
  switch (border_style_info.style) {
    ...
    case BorderStyle::kInset: {
      ...
      app_stream << GenerateColorAP(CFX_Color::Gray(left_top_gray_value),
      ...
      app_stream << GenerateColorAP(CFX_Color::Gray(right_bottom_gray_value),
                                    PaintOperation::kFill);
...
ByteString GetPopupContentsString(CPDF_Document* doc,
  ...
  ByteString color =
      GenerateColorAP(CFX_Color::RGB(0.0f, 0.0f, 0.0f), PaintOperation::kFill);
...
ByteString GenerateTextSymbolAP(const CFX_FloatRect& rect) {
  fxcrt::ostringstream app_stream;
  app_stream << GenerateColorAP(CFX_Color::RGB(1.0f, 1.0f, 0.0f),
                                PaintOperation::kFill);
  app_stream << GenerateColorAP(CFX_Color::RGB(0.0f, 0.0f, 0.0f),
...
ByteString GenerateComboBoxAP(const CPDF_Dictionary* annot_dict,
  ...
  ByteString button = GenerateColorAP(
  ...
  if (button.GetLength() > 0 && !button_rect.IsEmpty()) {
    ...
    ByteString button_border = GenerateBorderAP(
        button_rect, kButtonBorderStyleInfo, CFX_Color::Gray(0.0f));
  ...
  for (size_t i = top, sz = opts->size(); i < sz; i++) {
    ...
    if (RetainPtr<const CPDF_Object> opt = opts->GetDirectObjectAt(i)) {
      ...
      if (is_selected) {
        ...
        body_stream << "q\n"
                    << GenerateColorAP(CFX_Color::RGB(0.0f, 51.0f / 255.0f,
                                                      113.0f / 255.0f),
                                       PaintOperation::kFill);
        WriteRect(body_stream, item_rect) << " re f\nQ\n";
        body_stream << "BT\n"
                    << GenerateColorAP(CFX_Color::Gray(1.0f),
                                       PaintOperation::kFill)
...
bool GenerateCircleAP(CPDF_Document* doc, CPDF_Dictionary* annot_dict) {
  ...
  app_stream << GetColorStringWithDefault(
      interior_color.Get(), CFX_Color::Transparent(), PaintOperation::kFill);

  app_stream << GetColorStringWithDefault(
      annot_dict->GetArrayFor(pdfium::annotation::kC).Get(),
      CFX_Color::RGB(0.0f, 0.0f, 0.0f), PaintOperation::kStroke);
...
bool GenerateHighlightAP(CPDF_Document* doc, CPDF_Dictionary* annot_dict) {
  ...
  app_stream << GetColorStringWithDefault(
      annot_dict->GetArrayFor(pdfium::annotation::kC).Get(),
      CFX_Color::RGB(1.0f, 1.0f, 0.0f), PaintOperation::kFill);
