...
// on the same line into a single text rendering string. Reduces AP stream
// size but is disabled for RTL words where positioning is required.
ByteString GenerateEditAP(IPVT_FontMap* font_map,
  ...
  while (vt_iterator->NextWord()) {
    ...
    CPVT_Word peek_word;
    bool has_word = vt_iterator->GetWord(peek_word);

    bool use_continuous_formatting_for_word = use_continuous_formatting;
    if (use_continuous_formatting_for_word && has_word && peek_word.is_rtl()) {
    ...
    if (use_continuous_formatting_for_word) {
      if (place.LineCmp(oldplace) != 0) {
        ...
        if (has_word) {
          new_point = CFX_PointF(peek_word.location().x + offset.x,
                                 peek_word.location().y + offset.y);
        } else {
          CPVT_Line line;
      ...
      if (has_word) {
        if (peek_word.font_index() != current_font_index) {
          if (!words.IsEmpty()) {
            line_stream << GetWordRenderString(words.AsStringView());
            words.clear();
          }
          line_stream << GetFontSetString(font_map, peek_word.font_index(),
                                          peek_word.font_size());
          current_font_index = peek_word.font_index();
        }
        words += GetPDFWordString(font_map, current_font_index,
                                  peek_word.word(), sub_word);
      }
      oldplace = place;
    } else {
      if (!words.IsEmpty()) {
      ...
      if (has_word) {
        ...
        if (peek_word.font_index() != current_font_index) {
          edit_stream << GetFontSetString(font_map, peek_word.font_index(),
                                          peek_word.font_size());
          current_font_index = peek_word.font_index();
        }
        edit_stream << GetWordRenderString(
            GetPDFWordString(font_map, current_font_index, peek_word.word(),
                             sub_word)
                .AsStringView());
      }
    }
