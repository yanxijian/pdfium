...
TEST(SpanreadFD, ReadNormal) {
  ...
  const char kData[] = "hello world";
  {
    testing::ScopedFD fd(testing::OpenTempFileFD(
        temp_file->path(), testing::FileAccessMode::kWriteOnly));
  ...
  {
    testing::ScopedFD fd(testing::OpenTempFileFD(
        temp_file->path(), testing::FileAccessMode::kReadOnly));
    ASSERT_TRUE(fd.is_valid());
    char buffer[100];
...
TEST(SpanreadWin, ReadNormal) {
  ...
  const char kData[] = "hello world";
  {
    testing::ScopedPlatformHandle handle(testing::OpenTempFileHandle(
        temp_file->path(), testing::FileAccessMode::kWriteOnly));
    ASSERT_TRUE(handle.is_valid());
    DWORD bytes_written = 0;
    ASSERT_TRUE(::WriteFile(handle.get(), kData, sizeof(kData), &bytes_written,
                            nullptr));
