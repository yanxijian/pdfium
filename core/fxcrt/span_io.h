...
inline pdfium::span<T> spanread(Container&& dst, FILE* pFile) {
  ...
  size_t count = UNSAFE_BUFFERS(fread(s.data(), sizeof(T), s.size(), pFile));
  return s.first(count);
}

// Bounds-checked writes from a container into a file. Returns whether the
// entire container was successfully written.
template <typename Container>
[[nodiscard]] inline bool spanwrite(const Container& src, FILE* pFile) {
  ...
  return UNSAFE_BUFFERS(fwrite(s.data(), sizeof(T), s.size(), pFile)) ==
         s.size();
}
...
// Returns true if the entire span was successfully written.
template <typename Container>
[[nodiscard]] inline bool spanwrite(const Container& src, int fd) {
  auto s = pdfium::span(src);
  ...
  return bytes_written == static_cast<ssize_t>(s.size_bytes());
}
...
template <typename Container,
          typename T = typename decltype(pdfium::span(
              std::declval<Container>()))::element_type>
inline pdfium::span<T> spanread(Container&& dst, HANDLE hFile) {
  auto s = pdfium::span(std::forward<Container>(dst));
  DWORD bytes_read = 0;
  // SAFETY: ReadFile writes at most `s.size_bytes()` bytes into `s.data()`.
  // `s` guarantees `s.data()` has at least `s.size_bytes()` bytes.
  if (!::ReadFile(hFile, s.data(), static_cast<DWORD>(s.size_bytes()),
                  &bytes_read, nullptr)) {
    return {};
  }
  size_t elements_read = static_cast<size_t>(bytes_read) / sizeof(T);
...
// Returns true if the entire span was successfully written.
template <typename Container>
[[nodiscard]] inline bool spanwrite(const Container& src, HANDLE hFile) {
  auto s = pdfium::span(src);
  DWORD bytes_written = 0;
  // SAFETY: WriteFile reads at most `s.size_bytes()` bytes from `s.data()`.
  // `s` guarantees `s.data()` has at least `s.size_bytes()` bytes.
  if (!::WriteFile(hFile, s.data(), static_cast<DWORD>(s.size_bytes()),
                   &bytes_written, nullptr)) {
    return false;
  }
  return bytes_written == static_cast<DWORD>(s.size_bytes());
}
