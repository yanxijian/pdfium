...
class CFX_CSSStyleRule {
 public:
  ...
  CFX_CSSSelector* GetSelectorList(size_t index);
  const CFX_CSSSelector* GetSelectorList(size_t index) const {
    return const_cast<CFX_CSSStyleRule*>(this)->GetSelectorList(index);
  }
