...
pdfium::span<const uint8_t> GetFontTable(pdfium::span<const uint8_t> font_data,
                                         uint32_t tag,
                                         size_t face_offset) {
  if (font_data.size() < face_offset + 12) {
    return {};
  }
  auto face_data = font_data.subspan(face_offset);
  uint16_t nTables = fxcrt::GetUInt16MSBFirst(face_data.subspan<4, 2>());
  if (face_data.size() < 12u + static_cast<size_t>(nTables) * 16u) {
    return {};
  }
  auto table_dir = face_data.subspan(12u, static_cast<size_t>(nTables) * 16u);
