...
pdfium::span<const uint8_t> GetFontTable(pdfium::span<const uint8_t> font_data,
  ...
  uint16_t num_tables = fxcrt::GetUInt16MSBFirst(face_data.subspan<4u, 2u>());
  if (face_data.size() < 12u + num_tables * 16u) {
    return {};
  }
  auto table_dir = face_data.subspan(12u, num_tables * 16u);
