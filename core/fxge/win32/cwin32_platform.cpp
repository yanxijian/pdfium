...
class CFX_Win32FontInfo final : public SystemFontInfoIface {
 public:
  ...
  void AddInstalledFont(CFX_FontMapper* pMapper,
                        ByteString* pLastFamily,
                        const LOGFONTA* plf,
                        uint32_t font_type);

 private:
  bool IsSupportedFont(const LOGFONTA* plf);
  void GetGBPreference(CFX_FontMapper* pMapper,
                       ByteString& face,
                       int weight,
                       int pitch_family);
  void GetJapanesePreference(ByteString& face, int weight, int pitch_family);
  ByteString FindFont(CFX_FontMapper* pMapper, const ByteString& name);
...
void CFX_Win32FontInfo::AddInstalledFont(CFX_FontMapper* pMapper,
                                         ByteString* pLastFamily,
                                         const LOGFONTA* plf,
                                         uint32_t font_type) {
  ...
  if (name == *pLastFamily) {
    pMapper->AddInstalledFont(name, FX_GetCharsetFromInt(plf->lfCharSet));
    return;
  }
...
ByteString CFX_Win32FontInfo::FindFont(CFX_FontMapper* pMapper,
...
void* CFX_Win32FallbackFontInfo::MapFont(CFX_FontMapper* pMapper,
                                         int weight,
                                         bool bItalic,
                                         FX_Charset charset,
                                         int pitch_family,
...
void CFX_Win32FontInfo::GetGBPreference(CFX_FontMapper* pMapper,
                                        ByteString& face,
                                        int weight,
                                        int pitch_family) {
  if (face.Contains("KaiTi") || face.Contains("\xbf\xac")) {
    if (kai_ti_.IsEmpty()) {
      kai_ti_ = FindFont(pMapper, "KaiTi");
      if (kai_ti_.IsEmpty()) {
        kai_ti_ = "SimSun";
      }
    }
    ...
    if (fang_song_.IsEmpty()) {
      fang_song_ = FindFont(pMapper, "FangSong");
      if (fang_song_.IsEmpty()) {
        fang_song_ = "SimSun";
      }
    }
...
void* CFX_Win32FontInfo::MapFont(CFX_FontMapper* pMapper,
                                 int weight,
                                 bool bItalic,
                                 FX_Charset charset,
                                 int pitch_family,
  ...
  switch (charset) {
    case FX_Charset::kShiftJIS:
      ...
      break;
    case FX_Charset::kChineseSimplified:
      GetGBPreference(pMapper, new_face, weight, pitch_family);
      break;
    case FX_Charset::kHangul:
      new_face = "Gulim";
      break;
