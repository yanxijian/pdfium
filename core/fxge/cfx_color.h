...
struct CFX_Color {
  ...
  static constexpr CFX_Color Transparent() { return CFX_Color(); }
  static constexpr CFX_Color Gray(float g) { return CFX_Color(g); }
  static constexpr CFX_Color RGB(float r, float g, float b) {
    return CFX_Color(r, g, b);
  }
  static constexpr CFX_Color CMYK(float c, float m, float y, float k) {
    return CFX_Color(c, m, y, k);
  }
  static constexpr CFX_Color RGBBytes(int32_t r, int32_t g, int32_t b) {
    return CFX_Color(r / 255.0f, g / 255.0f, b / 255.0f);
  }
  static constexpr CFX_Color FromColorRef(FX_COLORREF ref) {
    return CFX_Color(FXARGB_R(ref) / 255.0f, FXARGB_G(ref) / 255.0f,
                     FXARGB_B(ref) / 255.0f);
  }
 ...
 private:
  constexpr explicit CFX_Color(float g) : nColorType(Type::kGray), fColor1(g) {}
  constexpr CFX_Color(float r, float g, float b)
      : nColorType(Type::kRGB), fColor1(r), fColor2(g), fColor3(b) {}
  constexpr CFX_Color(float c, float m, float y, float k)
      : nColorType(Type::kCMYK),
        fColor1(c),
        fColor2(m),
        fColor3(y),
        fColor4(k) {}
