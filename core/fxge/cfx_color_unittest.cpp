...
TEST(CFX_ColorTest, OperatorMinusUnusedComponents) {
  ...
  CFX_Color c1(CFX_Color::Type::kGray, 0.2f);
  ...
  CFX_Color c3(CFX_Color::Type::kGray, 0.7f);

  // Should be equal.
  EXPECT_EQ(c2, c3);

  // Construct a Gray color with non-zero unused components.
  CFX_Color c4(CFX_Color::Type::kGray, 0.7f, 0.5f, 0.5f, 0.5f);
...
TEST(CFX_ColorTest, OperatorSlashUnusedComponents) {
  // Construct a Gray color. Unused components (2, 3, 4) should be 0.0f.
  CFX_Color c1(CFX_Color::Type::kGray, 0.8f);
  ...
  CFX_Color c3(CFX_Color::Type::kGray, 0.4f);
  EXPECT_EQ(c2, c3);
}
