...
TEST(CFX_ColorTest, OperatorMinusUnusedComponents) {
  ...
  CFX_Color c1 = CFX_Color::Gray(0.2f);
  ...
  CFX_Color c3 = CFX_Color::Gray(0.7f);

  // Should be equal.
  EXPECT_EQ(c2, c3);

  // Construct a Gray color with non-zero unused components.
  CFX_Color c4 = CFX_Color::Gray(0.7f);
  c4.fColor2 = 0.5f;
  c4.fColor3 = 0.5f;
  c4.fColor4 = 0.5f;
...
TEST(CFX_ColorTest, OperatorSlashUnusedComponents) {
  // Construct a Gray color. Unused components (2, 3, 4) should be 0.0f.
  CFX_Color c1 = CFX_Color::Gray(0.8f);
  ...
  CFX_Color c3 = CFX_Color::Gray(0.4f);
  EXPECT_EQ(c2, c3);
}
