...
std::unique_ptr<CFX_GlyphBitmap> CFX_Face::RenderGlyph(
  ...
  if (subst_font) {
    subst_font->ApplySkew(&ft_matrix, font_style, is_vertical);
  ...
  if (subst_font) {
    int level = subst_font->GetEmboldenLevelForRender(font_style, ft_matrix);
    if (level < 0) {
      return nullptr;
    }
    if (level > 0) {
...
std::unique_ptr<CFX_Path> CFX_Face::LoadGlyphPath(
  ...
  if (subst_font) {
    subst_font->ApplySkew(&ft_matrix, /*font_style=*/false, is_vertical);
    if (subst_font->IsBuiltInGenericFont()) {
      AdjustVariationParams(glyph_index, dest_width, subst_font->weight_);
    }
  }
  ...
  if (subst_font) {
    int level = subst_font->GetEmboldenLevelForLoad(/*font_style=*/false);
    if (level > 0) {
