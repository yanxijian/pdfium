...
#if defined(PDF_ENABLE_SKIA_TYPEFACE_CHECKS) && defined(PDF_ENABLE_FONTATIONS)
bool RectsNear(const FX_RECT& r1, const FX_RECT& r2, int tolerance) {
  return std::abs(r1.left - r2.left) <= tolerance &&
         std::abs(r1.top - r2.top) <= tolerance &&
  ...
  if (IsTricky()) {
    ...
    if (err == 0) {
      ...
      if (skrifa_font_ && skrifa_font_->font->is_ok()) {
        ...
        CHECK(RectsNear(rect, skrifa_result, 2));
      }
#endif
#endif
    }
...
FX_RECT CFX_Face::GetGlyphBBox(uint32_t glyph_index) {
  ...
  if (skrifa_font_ && skrifa_font_->font->is_ok()) {
    ...
    FX_RECT skrifa_result(NormalizeFontMetric(bbox.x_min, upem),
                          ...
                          NormalizeFontMetric(bbox.x_max, upem),
                          NormalizeFontMetric(bbox.y_min, upem));
    CHECK(RectsNear(ft_result, skrifa_result, 1));
  }
#endif  // defined(PDF_ENABLE_FONTATIONS)
  if (skia_typeface_) {
