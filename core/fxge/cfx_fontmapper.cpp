      ...
      standard_faces_[base_font] =
          CFX_Face::New(nullptr,
                        pdfium::MakeRetain<CFX_ReadOnlySpanStream>(
                            CFX_StandardFont::GetStandardFont(
                                static_cast<CFX_StandardFont::ID>(base_font))),
                        0);
    }
...
RetainPtr<CFX_Face> CFX_FontMapper::FindSubstFace(const ByteString& name,
  ...
  if (subst_name == "Symbol" && !is_truetype) {
    ...
    return UseInternalSubst(static_cast<int>(CFX_StandardFont::kSymbol), weight,
                            italic_angle, 0, subst_font);
  }
  if (subst_name == "ZapfDingbats") {
    subst_font->family_ = "Chrome Dingbats";
    subst_font->charset_ = FX_Charset::kSymbol;
    return UseInternalSubst(static_cast<int>(CFX_StandardFont::kDingbats),
  ...
  std::optional<CFX_StandardFont::ID> std_font =
      CFX_StandardFont::GetStandardFontIndex(family);
  if (std_font.has_value() && std_font.value() < CFX_StandardFont::kSymbol) {
    base_font = static_cast<int>(std_font.value());
  } else {
    base_font = static_cast<int>(CFX_StandardFont::kSymbol);
  }
  int pitch_family;
  uint32_t nStyle;
  bool is_style_available = false;
  if (base_font < static_cast<int>(CFX_StandardFont::kSymbol)) {
  ...
  if (Charset == FX_Charset::kSymbol) {
    ...
    if (subst_name == "Symbol") {
      subst_font->family_ = "Chrome Symbol";
      subst_font->charset_ = FX_Charset::kSymbol;
      return UseInternalSubst(static_cast<int>(CFX_StandardFont::kSymbol),
