...
ByteString GetSubstName(const ByteString& name, bool is_truetype) {
  ...
  CFX_StandardFont::CanonicalizeStandardFontName(&subst_name);
  return subst_name;
}
...
RetainPtr<CFX_Face> CFX_FontMapper::FindSubstFace(const ByteString& name,
  ...
  std::optional<CFX_StandardFont::ID> std_font;
  {
    ...
    if (pos.has_value()) {
      ...
      std_font = CFX_StandardFont::CanonicalizeStandardFontName(&family);
  ...
  if (!std_font.has_value()) {
    std_font = CFX_StandardFont::GetStandardFontID(family);
  }
