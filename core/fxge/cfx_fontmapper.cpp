...
FX_Charset GetCharset(FX_CodePage code_page, int base_font, uint32_t flags) {
  ...
  if (FontStyleIsSymbolic(flags) && base_font == fxge::kNumStandardFonts) {
...
ByteString GetSubstName(const ByteString& name, bool is_truetype) {
  ...
  fxge::GetStandardFontName(&subst_name);
  return subst_name;
}
...
RetainPtr<CFX_Face> CFX_FontMapper::UseInternalSubst(
  ...
  if (base_font < fxge::kNumStandardFonts) {
    if (!standard_faces_[base_font]) {
      standard_faces_[base_font] = CFX_Face::New(
          nullptr,
          pdfium::MakeRetain<CFX_ReadOnlySpanStream>(fxge::GetStandardFont(
              static_cast<fxge::StandardFont>(base_font))),
          0);
    }
  ...
  if (!generic_sans_face_) {
    generic_sans_face_ = CFX_Face::New(
        nullptr,
        pdfium::MakeRetain<CFX_ReadOnlySpanStream>(fxge::GetGenericSansFont()),
        0);
  }
...
RetainPtr<CFX_Face> CFX_FontMapper::FindSubstFace(const ByteString& name,
  ...
  if (subst_name == "Symbol" && !is_truetype) {
    ...
    return UseInternalSubst(static_cast<int>(fxge::StandardFont::kSymbol),
                            weight, italic_angle, 0, subst_font);
  }
  if (subst_name == "ZapfDingbats") {
    subst_font->family_ = "Chrome Dingbats";
    subst_font->charset_ = FX_Charset::kSymbol;
    return UseInternalSubst(static_cast<int>(fxge::StandardFont::kDingbats),
                            weight, italic_angle, 0, subst_font);
  }
  int base_font = 0;
  ByteString family;
  ...
  bool has_hyphen = false;
  {
    ...
    if (pos.has_value()) {
      ...
      fxge::GetStandardFontName(&family);
  ...
  std::optional<fxge::StandardFont> std_font =
      fxge::GetStandardFontIndex(family);
  if (std_font.has_value() && std_font.value() < fxge::StandardFont::kSymbol) {
    base_font = static_cast<int>(std_font.value());
  } else {
    base_font = static_cast<int>(fxge::StandardFont::kSymbol);
  }
  int pitch_family;
  uint32_t nStyle;
  bool is_style_available = false;
  if (base_font < static_cast<int>(fxge::StandardFont::kSymbol)) {
    nStyle = GetStyleFromBaseFont(base_font);
    pitch_family = GetPitchFamilyFromBaseFont(base_font);
  } else {
    base_font = fxge::kNumStandardFonts;
  ...
  if (match.IsEmpty() && base_font >= fxge::kNumStandardFonts) {
    if (!is_cjk) {
  ...
  if (!match.IsEmpty() || base_font < fxge::kNumStandardFonts) {
    ...
    if (base_font < fxge::kNumStandardFonts) {
      ...
      family = fxge::GetCanonicalFontName(
          static_cast<fxge::StandardFont>(base_font));
    }
