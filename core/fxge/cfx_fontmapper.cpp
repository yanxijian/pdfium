...
FX_Charset GetCharset(FX_CodePage code_page, int base_font, uint32_t flags) {
  ...
  if (FontStyleIsSymbolic(flags) && base_font == FX_kNumStandardFonts) {
...
ByteString GetSubstName(const ByteString& name, bool is_truetype) {
  ...
  FX_GetStandardFontName(&subst_name);
  return subst_name;
}
...
RetainPtr<CFX_Face> CFX_FontMapper::UseInternalSubst(
  ...
  if (FontFamilyIsRoman(pitch_family)) {
    ...
    if (!generic_serif_face_) {
      generic_serif_face_ = CFX_Face::New(
          nullptr,
          pdfium::MakeRetain<CFX_ReadOnlySpanStream>(FX_GetGenericSerifFont()),
          0);
    }
  ...
  if (!generic_sans_face_) {
    generic_sans_face_ = CFX_Face::New(
        nullptr,
        pdfium::MakeRetain<CFX_ReadOnlySpanStream>(FX_GetGenericSansFont()), 0);
  }
...
RetainPtr<CFX_Face> CFX_FontMapper::FindSubstFace(const ByteString& name,
  ...
  if (subst_name == "Symbol" && !is_truetype) {
    ...
    return UseInternalSubst(static_cast<int>(StandardFont::kSymbol), weight,
                            italic_angle, 0, subst_font);
  }
  if (subst_name == "ZapfDingbats") {
    subst_font->family_ = "Chrome Dingbats";
    subst_font->charset_ = FX_Charset::kSymbol;
    return UseInternalSubst(static_cast<int>(StandardFont::kDingbats), weight,
                            italic_angle, 0, subst_font);
  }
  int base_font = 0;
  ByteString family;
  ...
  std::optional<StandardFont> std_font = FX_GetStandardFontIndex(family);
  if (std_font.has_value() && std_font.value() < StandardFont::kSymbol) {
    base_font = static_cast<int>(std_font.value());
  } else {
    base_font = static_cast<int>(StandardFont::kSymbol);
  }
  int pitch_family;
  uint32_t nStyle;
  bool is_style_available = false;
  if (base_font < static_cast<int>(StandardFont::kSymbol)) {
    nStyle = GetStyleFromBaseFont(base_font);
    pitch_family = GetPitchFamilyFromBaseFont(base_font);
  } else {
    base_font = FX_kNumStandardFonts;
  ...
  if (match.IsEmpty() && base_font >= FX_kNumStandardFonts) {
    if (!is_cjk) {
  ...
  if (!match.IsEmpty() || base_font < FX_kNumStandardFonts) {
    ...
    if (base_font < FX_kNumStandardFonts) {
      ...
      family = FX_GetCanonicalFontName(static_cast<StandardFont>(base_font));
    }
  ...
  if (Charset == FX_Charset::kSymbol) {
    ...
    if (subst_name == "Symbol") {
      subst_font->family_ = "Chrome Symbol";
      subst_font->charset_ = FX_Charset::kSymbol;
      return UseInternalSubst(static_cast<int>(StandardFont::kSymbol),
                              old_weight, italic_angle, pitch_family,
                              subst_font);
    }
#endif
