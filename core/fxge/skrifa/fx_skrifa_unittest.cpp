...
TEST(FxSkrifaTest, TestRobotoGlyph167Bounds) {
  ...
  EXPECT_LE(std::abs(ft_bbox.left - skrifa_rect.left), 1);
  EXPECT_LE(std::abs(ft_bbox.top - skrifa_rect.top), 1);
  EXPECT_LE(std::abs(ft_bbox.right - skrifa_rect.right), 1);
  EXPECT_LE(std::abs(ft_bbox.bottom - skrifa_rect.bottom), 1);
}
