...
RetainPtr<CFX_Type1Face> CFX_Type1Face::Create(
  ...
  return pdfium::WrapRetain(new CFX_Type1Face(
      std::move(cache_entry), std::move(font_stream), face_rec, std::move(skrifa_font)));
}

CFX_Type1Face::CFX_Type1Face(
    RetainPtr<Retainable> cache_entry,
    RetainPtr<CFX_ReadOnlySpanStream> font_stream,
    FT_FaceRec* rec,
    std::unique_ptr<SkrifaPsFontHolder> skrifa_font)
...
#endif
  return CFX_Face::LoadGlyphPath(glyph_index, dest_width, is_vertical, subst_font);
}
