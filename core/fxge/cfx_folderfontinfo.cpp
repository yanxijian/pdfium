...
void CFX_FolderFontInfo::ReportFace(const ByteString& path,
  ...
  const uint32_t nTables =
      fxcrt::GetUInt16MSBFirst(pdfium::span(buffer).subspan<4, 2>());
  auto table_dir_data = FixedSizeDataVector<uint8_t>::Uninit(nTables * 16);
  if (fxcrt::spanread(table_dir_data.span(), pFile).size() !=
      table_dir_data.size()) {
    return;
  }

  auto loc =
      FindFontTable(table_dir_data.span(), SystemFontInfoIface::kTableNAME);
  if (!loc) {
    return;
  }
  ...
  ByteString tables(ByteStringView(table_dir_data.span()));
