  ...
  if (bMatchName) {
    ...
    if (direct_it != font_list_.end()) {
      ...
      if (font->IsEligibleForFindFont(charset_flag, charset)) {
        ...
        pFind = font;
        if (CanReturnImmediately(font, iBestSimilar, charset)) {
          return font;
        }
      }
    }
...
bool CFX_FolderFontInfo::CanReturnImmediately(const FontFaceInfo* font,
                                              int32_t score,
                                              FX_Charset charset) const {
  return score == FontFaceInfo::kSimilarityScoreMax;
}
