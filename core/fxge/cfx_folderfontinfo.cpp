...
void CFX_FolderFontInfo::ReportFace(const ByteString& path,
  ...
  FX_SAFE_FILESIZE safe_name_end = loc->offset;
  safe_name_end += loc->size;
  if (!safe_name_end.IsValid() || safe_name_end.ValueOrDie() > filesize) {
    return;
  }
  ...
  if (maxp_loc) {
    FX_SAFE_FILESIZE safe_maxp_end = maxp_loc->offset;
    safe_maxp_end += maxp_loc->size;
    if (safe_maxp_end.IsValid() && safe_maxp_end.ValueOrDie() <= filesize) {
