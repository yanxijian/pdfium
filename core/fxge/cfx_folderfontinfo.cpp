...
void CFX_FolderFontInfo::ReportFace(const ByteString& path,
  ...
  if (os2.GetLength() >= 86) {
    ...
    if (codepages & (1U << 1)) {
      ...
      pInfo->charsets_ |= FX_SupportedCharsetFlag::kMSWin_EasternEuropean;
    }
    if (codepages & (1U << 2)) {
      mapper_->AddInstalledFont(facename, FX_Charset::kMSWin_Cyrillic);
      pInfo->charsets_ |= FX_SupportedCharsetFlag::kMSWin_Cyrillic;
    }
    if (codepages & (1U << 3)) {
      mapper_->AddInstalledFont(facename, FX_Charset::kMSWin_Greek);
      pInfo->charsets_ |= FX_SupportedCharsetFlag::kMSWin_Greek;
    }
    if (codepages & (1U << 4)) {
      mapper_->AddInstalledFont(facename, FX_Charset::kMSWin_Turkish);
      pInfo->charsets_ |= FX_SupportedCharsetFlag::kMSWin_Turkish;
    }
    if (codepages & (1U << 5)) {
      mapper_->AddInstalledFont(facename, FX_Charset::kMSWin_Hebrew);
      pInfo->charsets_ |= FX_SupportedCharsetFlag::kMSWin_Hebrew;
    }
    if (codepages & (1U << 6)) {
      mapper_->AddInstalledFont(facename, FX_Charset::kMSWin_Arabic);
      pInfo->charsets_ |= FX_SupportedCharsetFlag::kMSWin_Arabic;
    }
    if (codepages & (1U << 7)) {
      mapper_->AddInstalledFont(facename, FX_Charset::kMSWin_Baltic);
      pInfo->charsets_ |= FX_SupportedCharsetFlag::kMSWin_Baltic;
    }
    if (codepages & (1U << 16)) {
      mapper_->AddInstalledFont(facename, FX_Charset::kThai);
      pInfo->charsets_ |= FX_SupportedCharsetFlag::kThai;
    }
    if (codepages & (1U << 17)) {
    ...
    if (codepages & (1U << 18)) {
      ...
      pInfo->charsets_ |= FX_SupportedCharsetFlag::kChineseSimplified;
    }
    if (codepages & (1U << 20)) {
      mapper_->AddInstalledFont(facename, FX_Charset::kChineseTraditional);
      pInfo->charsets_ |= FX_SupportedCharsetFlag::kChineseTraditional;
    }
    if ((codepages & (1U << 19)) || (codepages & (1U << 21))) {
      mapper_->AddInstalledFont(facename, FX_Charset::kHangul);
      pInfo->charsets_ |= FX_SupportedCharsetFlag::kHangul;
    }
    if (codepages & (1U << 31)) {
      mapper_->AddInstalledFont(facename, FX_Charset::kSymbol);
      pInfo->charsets_ |= FX_SupportedCharsetFlag::kSymbol;
    }
  }
  mapper_->AddInstalledFont(facename, FX_Charset::kANSI);
  pInfo->charsets_ |= FX_SupportedCharsetFlag::kANSI;
  pInfo->styles_ = 0;
...
void* CFX_FolderFontInfo::FindFont(int weight,
  ...
  FX_SupportedCharsetFlag charset_flag = FX_GetSupportedCharsetFlag(charset);
