...
ByteString ReadStringFromFile(FILE* pFile, uint32_t size) {
  ByteString result;
  {
    ...
    if (fxcrt::spanread(buffer, pFile).size() != buffer.size()) {
      return ByteString();
    }
  }
...
void CFX_FolderFontInfo::ScanFile(const ByteString& path) {
  ...
  if (fxcrt::spanread(pdfium::span(buffer).first<12u>(), pFile.get()).size() !=
      12u) {
    return;
  }
  ...
  if (fxcrt::spanread(offsets_span, pFile.get()).size() !=
      offsets_span.size()) {
    return;
  }
...
void CFX_FolderFontInfo::ReportFace(const ByteString& path,
  ...
  if (fxcrt::spanread(pdfium::span(buffer).first<12u>(), pFile).size() != 12u) {
    return;
  }
