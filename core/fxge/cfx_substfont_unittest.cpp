...
TEST(CFXSubstFontTest, EffectiveWeight) {
  ...
  EXPECT_EQ(400, font.GetWeight(/*font_style=*/false));
  EXPECT_EQ(700, font.GetWeight(/*font_style=*/true));
}
...
TEST(CFXSubstFontTest, EmboldenLevels) {
  ...
  EXPECT_GT(font.GetEmboldenLevelForRender(/*font_style=*/false, 1024, 0), 0);
  EXPECT_GT(font.GetEmboldenLevelForLoad(), 0);
