...
void CompositeRowBgraPremul2BgraPremul(
  ...
  SkColorType dest_color_type = kBGRA_8888_SkColorType;
  if constexpr (std::is_same_v<DestPixelStruct, FX_RGBA_STRUCT<uint8_t>>) {
    dest_color_type = kRGBA_8888_SkColorType;
  }

  SkImageInfo src_info =
      SkImageInfo::Make(width, 1, kBGRA_8888_SkColorType, kPremul_SkAlphaType);
  SkImageInfo dest_info =
      SkImageInfo::Make(width, 1, dest_color_type, kPremul_SkAlphaType);

  SkPixmap src_pixmap(src_info, src_span.data(), src_info.minRowBytes());

  SkBitmap dest_bitmap;
  dest_bitmap.installPixels(dest_info, dest_span.data(),
                            dest_info.minRowBytes());
