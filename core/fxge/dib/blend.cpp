...
int BlendPremul(BlendMode blend_type,
  ...
  DCHECK_GE(back_color, 0);
  DCHECK_LE(back_color, 255);
  DCHECK_GE(back_alpha, 0);
  DCHECK_LE(back_alpha, 255);
  DCHECK_GE(src_color, 0);
  DCHECK_LE(src_color, 255);
  DCHECK_GE(src_alpha, 0);
  DCHECK_LE(src_alpha, 255);
  ...
  switch (blend_type) {
    ...
    default: {
      // This function does not handle non-separable blend modes or `kNormal`.
      NOTREACHED();
    }
  }
}
