...
void CFX_AndroidFontInfo::OnFaceReported(FontFaceInfo* pInfo,
  ...
  if (maxp.GetLength() >= 6) {
    ...
    pInfo->glyph_num_ = fxcrt::GetUInt16MSBFirst(p.first<2u>());
  }
}
...
bool CFX_AndroidFontInfo::IsBetterMatch(const FontFaceInfo* candidate,
  ...
  if (FX_CharSetIsCJK(charset)) {
    ...
    if (bMatchName) {
      ...
      matches_name = (candidate->face_name_ == family ||
                      FindFamilyNameMatch(bsFamily, candidate->face_name_));
    }
    uint32_t current_best_glyphs = current_best ? current_best->glyph_num_ : 0;
    return matches_name || candidate->glyph_num_ > current_best_glyphs;
  }
...
bool CFX_AndroidFontInfo::CanReturnImmediately(const FontFaceInfo* font,
                                               int32_t score,
                                               FX_Charset charset) const {
  if (FX_CharSetIsCJK(charset)) {
    return false;
  }
  return CFX_FolderFontInfo::CanReturnImmediately(font, score, charset);
}
