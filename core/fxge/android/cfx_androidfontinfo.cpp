...
void* CFX_AndroidFontInfo::MapFont(int weight,
  ...
  if (charset != FX_Charset::kMSWin_Arabic &&
    ...
    charset = FX_Charset::kMSWin_Arabic;
             ...
             SkiaMaybeSymbol(face.AsStringView())) {
  ...
  return FindFont(weight, bItalic, charset, pitch_family, face, !is_cjk);
}
    ...
    if (current_best) {
      bool current_best_matches =
          (current_best->face_name_ == family ||
           FindFamilyNameMatch(family_view, current_best->face_name_));
