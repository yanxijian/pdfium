...
void CFWL_ComboBox::RemoveStates(Mask<WidgetState> states) {
  if (IsDropDownStyle() && edit_) {
    edit_->RemoveStates(states);
  }
  if (list_box_) {
    list_box_->RemoveStates(states);
  }
  CFWL_Widget::RemoveStates(states);
}
...
void CFWL_ComboBox::ShowDropDownList() {
  ...
  list_box_->Update();
  list_box_->RemoveStates(WidgetState::kInvisible);
