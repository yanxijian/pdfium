...
void CXFA_FFPushButton::UpdateWidgetProperty() {
  ...
  switch (button_->GetHighlight()) {
    case XFA_AttributeValue::Inverted:
      highlight = fxcrt::to_underlying(HighlightStyle::kInverted);
      break;
    case XFA_AttributeValue::Outline:
      highlight = fxcrt::to_underlying(HighlightStyle::kOutline);
      break;
    case XFA_AttributeValue::Push:
      highlight = fxcrt::to_underlying(HighlightStyle::kPush);
      break;
    default:
      break;
  }
  GetNormalWidget()->ModifyStyleExts(highlight, 0xFFFFFFFF);
}
