...
void CFGAS_RTFBreak::EndBreakAlignment(const std::deque<TPO>& tpos,
  ...
  for (const TPO& pos : pdfium::Reversed(tpos)) {
    ...
    while (j > -1 && j < ttp.GetCharCount()) {
      ...
      if (tc->line_break_type_ == LineBreakType::kDirectBreak) {
        ++iGapChars;
      }
...
int32_t CFGAS_RTFBreak::GetBreakPos(std::vector<CFGAS_Char>& tca,
  ...
  --iLength;
  if (bAllChars) {
    pCur->line_break_type_ = LineBreakType::kUnknown;
  }
  ...
  while (iLength >= 0) {
    ...
    if (nCur == BreakProperty::kTB) {
      ...
      eType = nNext == BreakProperty::kTB
                  ? LineBreakType::kProhibitedBreak
    ...
    if (!bOnlyBrk) {
      ...
      if (*pEndPos <= line_width_ || bNeedBreak) {
        if (eType == LineBreakType::kDirectBreak && iBreak < 0) {
          iBreak = iLength;
          iBreakPos = *pEndPos;
          if (!bAllChars) {
            return iLength;
          }
        } else if (eType == LineBreakType::kIndirectBreak && iIndirect < 0) {
          iIndirect = iLength;
...
void CFGAS_RTFBreak::SplitTextLine(CFGAS_BreakLine* pCurLine,
  ...
  if (iCharPos >= fxcrt::CollectionSize<int32_t>(pCurLine->line_chars_)) {
    pNextLine->Clear();
    curChars[iCharPos - 1].line_break_type_ = LineBreakType::kUnknown;
    return;
  }
