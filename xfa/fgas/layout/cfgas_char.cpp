...
BidiClass GetDeferredType(int32_t val) {
  return static_cast<BidiClass>((val >> 4) & 0x0F);
}

BidiClass GetResolvedType(int32_t val) {
  return static_cast<BidiClass>(val & 0x0F);
}
...
void ResolveWeak(std::vector<CFGAS_Char>* chars, size_t iCount) {
  ...
  for (; i <= iCount; ++i) {
    ...
    if (eClsRun != static_cast<BidiClass>(0xF) && iNum > 0) {
      SetDeferredRunClass(chars, i, iNum, eClsRun);
      iNum = 0;
    }
    eClsNew = GetResolvedType(eAction);
    if (eClsNew != static_cast<BidiClass>(0xF)) {
  ...
  if (eClsRun != static_cast<BidiClass>(0xF)) {
    SetDeferredRunClass(chars, i, iNum, eClsRun);
  }
}
